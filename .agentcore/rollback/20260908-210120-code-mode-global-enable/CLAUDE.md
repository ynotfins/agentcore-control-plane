...
- Non-Swarm clients use one AgentCore MCP entry named `agentcore-gateway`. Header-capable
  clients use the direct `:8080` route with environment-backed authentication. Approved
  header-hostile Trust Class A clients use the process-attested `:18082` route selected by
  `contracts/agentcore-gateway-client.json`; they do not store the builder bearer.
