...
| Property | Value |
|----------|-------|
| **Primary storage** | PostgreSQL 18 `agent_core` — `agentcore.wf_evidence`, `agentcore.memory_events` (F: NVMe) |
...
| **Archive trigger** | After 90 days in PostgreSQL, export to E: (compressed JSON). Original PostgreSQL rows preserved. |
...
| Property | Value |
|----------|-------|
| **Primary storage** | PostgreSQL 18 `agent_core` — `agentcore.memory_summaries` (F: NVMe) |
| **Archive location** | E: (included in logical pg_dump backups) |
| **Retention duration** | Indefinite — all levels kept |
| **Deletion policy** | NO automatic deletion. Summaries are re-generated from immutable_evidence if corrupted. |
...
Any evidence row that enters `agentcore.wf_evidence` or `agentcore.memory_events`
