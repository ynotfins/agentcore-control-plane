...
- `arabold-docs`, `serena`, `sequential-thinking`, `artiforge`, `obsidian-vault`, `context-fabric` per the governed shapes in `contracts/master-mcp-server-config.json`. Serena must use the installed `serena.exe` launcher with `start-mcp-server --transport stdio --context claude-code --project-from-cwd`; do not use git `uvx` or `--context ide-assistant`. Use Windows env var references only; no `.env`. Do not add `global-memory-gateway` unless the current source authority explicitly contains it.
...
ENFORCE the universal rule contract (see `docs/prompts/README.md`): native recall via `swarmrecall`; RAG/context via `swarmvault`; project code intelligence via `serena`; `arabold-docs` not context7; never raw-SQL or dual-write; never write `F:\AgentCore` by filesystem; never store secrets.
