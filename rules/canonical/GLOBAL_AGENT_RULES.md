...
**Source of truth:** `contracts/global-agent-policy.yaml` (policy_revision 2026-07-31).
...
3. **Swarm isolation.** SwarmRecall, SwarmVault, and SwarmClaw are a separate ecosystem. Never require Swarm MCP servers, Swarm databases, or Swarm file roots for AgentCore / enrolled non-Swarm work. Never modify Swarm product installs. OpenClaw/ClawX are Swarm-managed. No normal AgentCore IDE continuity on Swarm projects — HARD STOP with `swarm_project_refused`. Do not persist Swarm work through AgentCore memory, project router, or IDE profiles. A neutral dual workspace may be used only for read-only collision and boundary audits. SwarmRecall is Swarm’s required memory/context layer; do not claim it replaces SwarmClaw native application databases unless proven under Swarm authority.
