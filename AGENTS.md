...
- Cursor Stage A new-chat recovery uses project hooks (`sessionStart`, `beforeSubmitPrompt` only) via `scripts/agentcore_cursor/hook_dispatcher.py`; `preToolUse` is offline-tested but not registered until operator acceptance. Runbook: `docs/operations/AUTOMATIC_NEW_CHAT_RECOVERY.md`.
