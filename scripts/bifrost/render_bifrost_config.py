...
STATIC_ENV_VALUES: dict[str, dict[str, str]] = {
    "exa-research": {"AGNOST_DISABLE_LOGS": "true"},
    "arabold-docs": {
...
class OutputSchemaWiring:
    ...
    def wrap(
        ...
        alias_sources = set(env_aliases.values())
        transform_args = (
        ...
        wrapped_args = [
            ...
            *[
                ...
                if env_name not in alias_sources
                for argument in ("--allow-env", env_name)
            ],
            *[
                argument
                for child_name, parent_name in sorted(env_aliases.items())
                for argument in ("--alias-env", f"{child_name}={parent_name}")
            ],
            *[
                argument
...
def build_stdio_client(
    ...
    command, args = output_schema.wrap(
        ...
        dict(server.get("env_aliases") or {}),
        static_env,
    )
    ...
    notes: dict[str, Any] = {
        ...
        "required_parent_env": sorted(
            set(server.get("env_var_names") or [])
            | set((server.get("env_aliases") or {}).values())
        ),
        "env_aliases": dict(server.get("env_aliases") or {}),
...
def build_virtual_keys(registry: dict[str, Any]) -> list[dict[str, Any]]:
    return [
        ...
        {
            "id": "vk-agentcore-openclaw",
            "name": "openclaw",
            "value": "env.BIFROST_MCP_VK_OPENCLAW",
            "is_active": True,
            "mcp_configs": profile_mcp_configs(registry, "openclaw"),
        },
    ]
