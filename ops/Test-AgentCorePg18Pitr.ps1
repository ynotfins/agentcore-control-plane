...
SELECT project_id, source_identity_id, 'accepted_evidence', '$marker',
       jsonb_build_object('pitr_marker', '$marker'),
       'system_verified',
       jsonb_build_object('test', 'pg18_pitr', 'created_by', 'Test-AgentCorePg18Pitr.ps1')
FROM selected
...
if (-not (Test-Path -LiteralPath $switchedPath)) { throw "Switched WAL was not archived: $switchedWal" }
...
try {
  ...
  $reportPath = Join-Path $evidencePath "pg18-pitr-test-$stamp.json"
