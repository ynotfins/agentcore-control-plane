/**
 * Serena MCP Pre-warm Proxy for Bifrost — correct bridging with ping interception
 * 
 * Serena does not implement MCP ping. Ping responses are always synthetic.
 * All other calls are forwarded to Serena once it is ready.
 * 
...
function handleBifrostLine(line) {
  ...
  let msg; try { msg = JSON.parse(line); } catch(e) {
    if (state === "bridging") toSerena(line); else pendingMessages.push(line);
    return;
  }

  // ALWAYS respond to ping synthetically (Serena does not support MCP ping)
  if (msg.method === "ping") {
    toBifrost({ jsonrpc: "2.0", id: msg.id, result: {} });
    return;
  }
  ...
  // Pre-bridging synthetic responses
  if (msg.method === "initialize" && !servedInit) {
    servedInit = true;
    toBifrost({ jsonrpc:"2.0", id:msg.id, result:{
      protocolVersion:"2025-03-26",
      capabilities:{ tools:{listChanged:true}, prompts:{}, resources:{} },
      serverInfo:{ name:"serena", version:"1.5.4.dev0" }
    }});
    log("Synthetic initialize id=" + msg.id);
    return;
  }
  if (msg.method === "tools/list" && !servedTools) {
    servedTools = true;
    toBifrost({ jsonrpc:"2.0", id:msg.id, result:{ tools:CACHED_TOOLS, nextCursor:null }});
    log("Synthetic tools/list (" + CACHED_TOOLS.length + " tools) id=" + msg.id);
    return;
  }
  // Buffer everything else until bridging
  pendingMessages.push(line);
}

// SINGLE stdin listener - handles all states via handleBifrostLine
process.stdin.on("data", function(chunk) {
  inboundBuf += chunk.toString();
  const lines = inboundBuf.split("\n");
  inboundBuf = lines.pop(); // Keep incomplete line in buffer
  for (const line of lines) handleBifrostLine(line);
});
